    ...
    st.markdown(
        ...
        <style>
            ...
            @media (max-width: 768px) {{
                .section-card, .detail-card, .sir-card {{ padding: 0.85rem; border-radius: 14px; }}
            }}
        </style>
        """,
...
def mostrar_ficha_registro(tabla, registro):
    ...
    st.markdown(f"### Ficha completa · {config['titulo']}")
    ...
    for campo_chip in ["nivel_prioridad_social", "nivel", "estado", "prioridad", "validada", "requiere_medida_diferencial", "condicion_discapacidad"]:
        if campo_chip in registro.index:
            ...
            chips.append(crear_chip(f"{etiqueta_campo(campo_chip)}: {valor}", tipo_chip_por_valor(valor)))
    if chips:
    ...
    if st.button("Editar este registro", key=f"btn_editar_{tabla}_{id_registro}", use_container_width=True):
...
def mostrar_ficha_resumen_hogar(id_hogar):
    """Muestra ficha ejecutiva del hogar seleccionado."""
    ...
    st.markdown(
        crear_chip(f"Prioridad: {hogar.get('nivel_prioridad_social', 'No registrado')}", tipo_chip_por_valor(hogar.get('nivel_prioridad_social', ''))) +
        crear_chip(f"LB validada: {validada}", tipo_chip_por_valor(validada)) +
        crear_chip(f"Vulnerabilidades: {total_vul}", "danger" if total_vul else "success"),
        unsafe_allow_html=True,
    )
