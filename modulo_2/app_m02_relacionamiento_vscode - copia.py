...
def generar_id(prefijo):
    """Genera un ID corto de prueba para registros internos."""
...
def inicializar_estado():
    """Inicializa dataframes y variables de control en memoria de sesión."""
    ...
    if "actores_clave" not in st.session_state:
        ...
        st.session_state.actores_clave = actores
        st.session_state.interacciones = interacciones
        st.session_state.participantes_interaccion = participantes
        st.session_state.seguimiento_interacciones = seguimientos
...
def guardar_registro(nombre_tabla, columna_id, registro):
    """Guarda un registro nuevo o actualizado dentro del dataframe correspondiente."""
    ...
    if vacios:
        st.warning(f"Registro guardado, pero está incompleto. Campos vacíos: {', '.join(vacios)}")
    else:
...
# ===============================================================
# 06. COMPONENTES VISUALES GENERALES
# ===============================================================
...
def formulario_participantes(registro=None):
    ...
    with st.form("form_participantes", clear_on_submit=False):
        ...
        if guardar:
            ...
            if registro:
                mascara = (
                    (df["id_interaccion"].astype(str) == str(registro.get("id_interaccion", ""))) &
                    (df["id_persona"].astype(str) == str(registro.get("id_persona", ""))) &
                    (df["id_actor"].astype(str) == str(registro.get("id_actor", ""))) &
                    (df["nombre_participante_externo"].astype(str) == str(registro.get("nombre_participante_externo", "")))
                )
                if mascara.any():
                    df.loc[mascara, list(nuevo_registro.keys())] = list(nuevo_registro.values())
                else:
                    df = pd.concat([df, pd.DataFrame([nuevo_registro])], ignore_index=True)
            else:
                df = pd.concat([df, pd.DataFrame([nuevo_registro])], ignore_index=True)

            st.session_state.participantes_interaccion = df
            vacios = campos_vacios(nuevo_registro)
            if vacios:
                st.warning(f"Registro guardado, pero está incompleto. Campos vacíos: {', '.join(vacios)}")
            else:
                st.success("Registro guardado completo.")
...
def pantalla_participantes(filtro_hogar, filtro_predio):
    ...
    columnas = ["id_interaccion", "id_persona", "id_actor", "nombre_participante_externo", "tipo_participante", "rol_participante", "firma_asistencia"]
