...
def mostrar_filtros_globales():
    """Muestra filtros generales por hogar, predio, actor y zona."""
    ...
    with col3:
        filtro_actor = st.selectbox("Filtrar por código de actor", obtener_opciones_columna(actores, "id_actor"), key="filtro_actor_global")
    with col4:
        filtro_zona = st.selectbox("Filtrar por zona", zonas, key="filtro_zona_global")

    return filtro_hogar, filtro_predio, filtro_actor, filtro_zona


def aplicar_filtros(df, filtro_hogar="", filtro_predio="", filtro_actor="", filtro_zona=""):
    """Aplica filtros generales, incluso cuando la relación es indirecta entre tablas."""
    resultado = df.copy()
    hogares = st.session_state.catalogos["hogares"]
    actores = st.session_state.actores_clave
    ...
    # Filtro por predio: se traduce a hogares relacionados.
    if filtro_predio:
        hogares_filtrados = hogares[hogares["codigo_predio"].astype(str) == str(filtro_predio)]["id_hogar"].astype(str).tolist()
        if "id_hogar" in resultado.columns:
            resultado = resultado[resultado["id_hogar"].astype(str).isin(hogares_filtrados)]
        elif "id_interaccion" in resultado.columns:
            interacciones = st.session_state.interacciones
            ids_interacciones = interacciones[interacciones["id_hogar"].astype(str).isin(hogares_filtrados)]["id_interaccion"].astype(str).tolist()
            resultado = resultado[resultado["id_interaccion"].astype(str).isin(ids_interacciones)]

    # Filtro por hogar directo o por interacciones asociadas.
    if filtro_hogar:
        if "id_hogar" in resultado.columns:
            resultado = resultado[resultado["id_hogar"].astype(str) == str(filtro_hogar)]
        elif "id_interaccion" in resultado.columns:
            interacciones = st.session_state.interacciones
            ids_interacciones = interacciones[interacciones["id_hogar"].astype(str) == str(filtro_hogar)]["id_interaccion"].astype(str).tolist()
            resultado = resultado[resultado["id_interaccion"].astype(str).isin(ids_interacciones)]

    # Filtro por actor directo o por interacciones en las que participó.
    if filtro_actor:
    ...
    # Filtro por zona: se resuelve por lugar poblado o por hogar/interacción asociada.
    if filtro_zona:
        lugares_filtrados = obtener_lugares_por_zona(filtro_zona)
        hogares_zona = hogares[hogares["id_lugar_poblado"].astype(str).isin(lugares_filtrados)]["id_hogar"].astype(str).tolist()
...
def pantalla_actores_clave(filtro_hogar, filtro_predio, filtro_actor, filtro_zona):
    """Pantalla de gestión de actores clave."""
    st.subheader("Actores clave")
    df = aplicar_filtros(st.session_state.actores_clave, filtro_hogar, filtro_predio, filtro_actor, filtro_zona)
    ...
    if indice is not None and not df.empty:
        ...
        st.markdown('<div class="descarga-box"><b>Descarga del actor seleccionado</b></div>', unsafe_allow_html=True)
        df_actor = convertir_registro_a_df(registro)
        col_csv_actor, col_pdf_actor = st.columns(2)
        with col_csv_actor:
            st.download_button(
                label="Descargar CSV del actor seleccionado",
                data=convertir_df_a_csv(df_actor),
                file_name=f"m02_actor_{registro.get('id_actor', 'seleccionado')}.csv",
                mime="text/csv",
                use_container_width=True,
            )
        with col_pdf_actor:
            st.download_button(
                label="Descargar PDF del actor seleccionado",
                data=convertir_df_a_pdf(df_actor, f"Actor clave seleccionado - {registro.get('id_actor', '')}"),
                file_name=f"m02_actor_{registro.get('id_actor', 'seleccionado')}.pdf",
                mime="application/pdf",
                use_container_width=True,
                disabled=not REPORTLAB_DISPONIBLE,
            )
...
def pantalla_interacciones(filtro_hogar, filtro_predio, filtro_actor, filtro_zona):
    """Pantalla de gestión de interacciones."""
    st.subheader("Interacciones")
    df = aplicar_filtros(st.session_state.interacciones, filtro_hogar, filtro_predio, filtro_actor, filtro_zona)
...
def pantalla_seguimiento_interacciones(filtro_hogar, filtro_predio, filtro_actor, filtro_zona):
    ...
    interacciones_filtradas = aplicar_filtros(st.session_state.interacciones, filtro_hogar, filtro_predio, filtro_actor, filtro_zona)
    ...
    if filtro_hogar or filtro_predio or filtro_actor or filtro_zona:
...
def pantalla_participantes(filtro_hogar, filtro_predio, filtro_actor, filtro_zona):
...
def main():
    ...
    filtro_hogar, filtro_predio, filtro_actor, filtro_zona = mostrar_filtros_globales()
    st.divider()

    if tabla == "Actores clave":
        pantalla_actores_clave(filtro_hogar, filtro_predio, filtro_actor, filtro_zona)
    elif tabla == "Interacciones":
        pantalla_interacciones(filtro_hogar, filtro_predio, filtro_actor, filtro_zona)
    elif tabla == "Seguimiento de interacciones":
        pantalla_seguimiento_interacciones(filtro_hogar, filtro_predio, filtro_actor, filtro_zona)
    else:
        pantalla_participantes(filtro_hogar, filtro_predio, filtro_actor, filtro_zona)
