...
def cargar_estilos():
    ...
    st.markdown(
        ...
        .tag-chip {
            ...
            border: 1px solid rgba(30,138,138,0.32);
            color: inherit;
            font-size: 0.88rem;
            font-weight: 600;
        }
        ...
        .ficha-title { font-size: 1.1rem; font-weight: 800; color: inherit; margin: 0; }
        .ficha-id {
            border-radius: 999px;
            padding: 0.28rem 0.7rem;
            background: rgba(30,138,138,0.14);
            border: 1px solid rgba(30,138,138,0.35);
            font-weight: 800;
            white-space: nowrap;
        }
        .ficha-section-title {
...
def cargar_catalogos_base():
    ...
    return {
        "lugares_poblados": pd.DataFrame([
            {"id_lugar_poblado": "COM-001", "nombre_lugar_poblado": "Comunidad Río Indio", "distrito": "Capira", "zona": "Zona 1"},
            {"id_lugar_poblado": "COM-002", "nombre_lugar_poblado": "Comunidad La Mina", "distrito": "Colón", "zona": "Zona 2"},
            {"id_lugar_poblado": "COM-003", "nombre_lugar_poblado": "Comunidad Alto del Río", "distrito": "Panamá Oeste", "zona": "Zona 3"},
        ]),
...
def inicializar_estado():
    ...
    if "actores_clave" not in st.session_state:
        ...
        if "participante_en_base" not in st.session_state.participantes_interaccion.columns:
            st.session_state.participantes_interaccion.insert(2, "participante_en_base", "")
            mascara_en_base = (
                st.session_state.participantes_interaccion.get("id_persona", "").astype(str).str.strip().ne("") |
                st.session_state.participantes_interaccion.get("id_actor", "").astype(str).str.strip().ne("")
            )
            st.session_state.participantes_interaccion.loc[mascara_en_base, "participante_en_base"] = "Sí"
            st.session_state.participantes_interaccion.loc[~mascara_en_base, "participante_en_base"] = "No"
            guardar_tabla_local("participantes_interaccion")
...
def normalizar_temas(valor):
    """Convierte texto de temas a una lista limpia de etiquetas sin duplicados."""
    if valor is None:
        return []
    texto = str(valor).replace("|", ",").replace(";", ",")
    temas = []
    for tema in texto.split(","):
        tema_limpio = tema.strip()
        if tema_limpio and tema_limpio not in temas:
            temas.append(tema_limpio)
    return temas


def temas_a_texto(temas):
    """Guarda las etiquetas de temas tratados en un formato simple y legible."""
    return " | ".join([str(tema).strip() for tema in temas if str(tema).strip()])


def mostrar_chips_temas(temas):
    """Muestra etiquetas visuales para los temas tratados capturados manualmente."""
    if not temas:
        st.caption("Sin temas registrados.")
        return
    chips = "".join([f'<span class="tag-chip">{tema}</span>' for tema in temas])
    st.markdown(chips, unsafe_allow_html=True)


def obtener_zona_por_lugar(id_lugar_poblado):
    """Obtiene la zona asociada a un lugar poblado desde el catálogo."""
    lugares = st.session_state.catalogos.get("lugares_poblados", pd.DataFrame())
    if lugares.empty or "zona" not in lugares.columns:
        return ""
    registro = obtener_registro_por_id(lugares, "id_lugar_poblado", id_lugar_poblado)
    return registro.get("zona", "") if registro else ""


def obtener_lugares_por_zona(zona):
    """Devuelve los lugares poblados que pertenecen a una zona."""
    lugares = st.session_state.catalogos.get("lugares_poblados", pd.DataFrame())
    if not zona or lugares.empty or "zona" not in lugares.columns:
        return []
    return lugares[lugares["zona"].astype(str) == str(zona)]["id_lugar_poblado"].astype(str).tolist()


def obtener_interacciones_por_actor(id_actor):
    """Devuelve interacciones relacionadas con un actor seleccionado desde participantes."""
    participantes = st.session_state.get("participantes_interaccion", pd.DataFrame())
    if not id_actor or participantes.empty or "id_actor" not in participantes.columns:
        return []
    return participantes[participantes["id_actor"].astype(str) == str(id_actor)]["id_interaccion"].astype(str).tolist()


def convertir_registro_a_df(registro):
    """Convierte un registro seleccionado a dataframe para descargas unitarias."""
    if not registro:
        return pd.DataFrame()
    return pd.DataFrame([registro]).fillna("")
...
def mostrar_descarga_registro_seleccionado(registro, nombre_archivo_base, titulo_pdf, etiqueta="registro seleccionado"):
    """Muestra botones para descargar únicamente el registro seleccionado en CSV y PDF."""
    if not registro:
        return

    df_registro = convertir_registro_a_df(registro)
    identificador = (
        registro.get("id_actor")
        or registro.get("id_interaccion")
        or registro.get("id_seguimiento")
        or registro.get("id_participante")
        or "seleccionado"
    )
    nombre_seguro = re.sub(r"[^A-Za-z0-9_-]+", "_", str(identificador)).strip("_") or "seleccionado"

    st.markdown(f'<div class="descarga-box"><b>Descarga del {etiqueta}</b></div>', unsafe_allow_html=True)
    col_csv, col_pdf = st.columns(2)
    with col_csv:
        st.download_button(
            label=f"Descargar CSV del {etiqueta}",
            data=convertir_df_a_csv(df_registro),
            file_name=f"{nombre_archivo_base}_{nombre_seguro}.csv",
            mime="text/csv",
            use_container_width=True,
        )
    with col_pdf:
        st.download_button(
            label=f"Descargar PDF del {etiqueta}",
            data=convertir_df_a_pdf(df_registro, f"{titulo_pdf} - {identificador}") if REPORTLAB_DISPONIBLE else b"",
            file_name=f"{nombre_archivo_base}_{nombre_seguro}.pdf",
            mime="application/pdf",
            use_container_width=True,
            disabled=not REPORTLAB_DISPONIBLE,
        )
...
def mostrar_filtros_globales():
    """Muestra filtros generales solicitados: código de actor y zona."""
    actores = st.session_state.actores_clave
    lugares = st.session_state.catalogos["lugares_poblados"]
    zonas = obtener_opciones_columna(lugares, "zona") if "zona" in lugares.columns else [""]

    col1, col2 = st.columns(2)
    with col1:
        filtro_actor = st.selectbox(
            "Filtrar por código de actor",
            obtener_opciones_columna(actores, "id_actor"),
            key="filtro_actor_global",
        )
    with col2:
        filtro_zona = st.selectbox(
            "Filtrar por zona",
            zonas,
            key="filtro_zona_global",
        )

    return filtro_actor, filtro_zona


def aplicar_filtros(df, filtro_actor="", filtro_zona=""):
    """Aplica filtros por actor y zona respetando relaciones directas e indirectas entre tablas."""
    resultado = df.copy()
    ...
    if filtro_actor:
        if "id_actor" in resultado.columns:
            resultado = resultado[resultado["id_actor"].astype(str) == str(filtro_actor)]
        elif "id_interaccion" in resultado.columns:
            ids_interacciones = obtener_interacciones_por_actor(filtro_actor)
            resultado = resultado[resultado["id_interaccion"].astype(str).isin(ids_interacciones)]

    if filtro_zona:
        lugares_filtrados = obtener_lugares_por_zona(filtro_zona)
        hogares_zona = hogares[
            hogares["id_lugar_poblado"].astype(str).isin(lugares_filtrados)
        ]["id_hogar"].astype(str).tolist()

        if "id_lugar_poblado" in resultado.columns:
            resultado = resultado[resultado["id_lugar_poblado"].astype(str).isin(lugares_filtrados)]
        elif "id_hogar" in resultado.columns:
            resultado = resultado[resultado["id_hogar"].astype(str).isin(hogares_zona)]
        elif "id_interaccion" in resultado.columns:
            interacciones = st.session_state.interacciones
            ids_interacciones = interacciones[
                interacciones["id_lugar_poblado"].astype(str).isin(lugares_filtrados) |
                interacciones["id_hogar"].astype(str).isin(hogares_zona)
            ]["id_interaccion"].astype(str).tolist()
            resultado = resultado[resultado["id_interaccion"].astype(str).isin(ids_interacciones)]
...
def mostrar_ficha_resumen(titulo, registro):
    """Muestra una ficha profesional tipo tarjeta, no una tabla."""
    if not registro:
        return
    ...
    campos_prioritarios = [
        "nombre_actor", "nombre_participante_externo", "tipo_actor", "tipo_interaccion",
        "estado_relacionamiento", "estado_seguimiento", "nivel_influencia",
        "nivel_sensibilidad", "resultado", "requiere_seguimiento",
    ]
    campos_identificacion = [
        "id_actor", "id_interaccion", "id_seguimiento", "id_participante",
        "id_persona", "id_hogar", "id_lugar_poblado",
    ]
    campos_fechas = ["fecha_interaccion", "hora_inicio", "hora_fin", "fecha_registro", "fecha_compromiso"]
    campos_gestion = [
        "rol_interes", "motivo", "temas_tratados", "solicitudes_hogar", "acuerdos",
        "actividades_acciones", "accion_seguimiento", "observaciones",
        "responsable_registro", "responsable_seguimiento", "evidencia_principal",
        "evidencia_seguimiento", "firma_asistencia", "participante_en_base",
        "tipo_participante", "rol_participante", "canal", "tipo_reunion",
    ]

    usados = set()

    def construir_items(campos):
        html = ""
        for campo in campos:
            if campo in registro and campo not in usados:
                usados.add(campo)
                valor = limpiar_texto(registro.get(campo, "")) or "—"
                if campo == "temas_tratados":
                    temas = normalizar_temas(valor)
                    valor = " · ".join(temas) if temas else "—"
                html += f'<div class="ficha-item"><span class="ficha-label">{etiqueta(campo)}</span><span class="ficha-value">{valor}</span></div>'
        return html

    secciones = []
    prioritarios = construir_items(campos_prioritarios)
    if prioritarios:
        secciones.append(("Resumen ejecutivo", prioritarios))
    identificacion = construir_items(campos_identificacion)
    if identificacion:
        secciones.append(("Identificación y relaciones", identificacion))
    fechas = construir_items(campos_fechas)
    if fechas:
        secciones.append(("Fechas y control", fechas))
    gestion = construir_items(campos_gestion)
    if gestion:
        secciones.append(("Gestión y trazabilidad", gestion))

    restantes = construir_items([campo for campo in registro.keys() if campo not in usados])
    if restantes:
        secciones.append(("Otros campos", restantes))

    html_secciones = ""
    for nombre, contenido in secciones:
        html_secciones += f'<div class="ficha-section-title">{nombre}</div><div class="ficha-grid">{contenido}</div>'
    ...
    st.markdown(
        ...
        <div class="ficha-profesional">
            <div class="ficha-header">
                <div>
                    <p class="ficha-title">Ficha del registro seleccionado</p>
                    <span>Vista ejecutiva para revisión rápida y trazabilidad operativa.</span>
                </div>
                <div class="ficha-id">{id_principal}</div>
            </div>
            {html_secciones}
        </div>
        ''',
...
def formulario_actores_clave(registro=None):
    ...
    with st.form("form_actores_clave", clear_on_submit=False):
        ...
        with col1:
            ...
            id_actor = st.text_input("ID actor", value=registro.get("id_actor", generar_id("ACTOR")), disabled=True, help="ID automático generado por el sistema.", label_visibility="collapsed")
...
def formulario_interacciones(registro=None):
    ...
    with st.form("form_interacciones", clear_on_submit=False):
        ...
        with col1:
            ...
            id_interaccion = st.text_input("ID interacción", value=registro.get("id_interaccion", generar_id("INT")), disabled=True, help="ID automático generado por el sistema.", label_visibility="collapsed")
            ...
            zona_lugar = obtener_zona_por_lugar(id_lugar)
            if zona_lugar:
                st.caption(f"Zona asociada: {zona_lugar}")
        ...
        temas_actuales = normalizar_temas(registro.get("temas_tratados", ""))
        temas_captura = st.text_input(
            "Temas tratados",
            value=", ".join(temas_actuales),
            placeholder="Ejemplo: derechos, acuerdos, queja, visita técnica",
            help="Ingresa cada tema separado por coma. Cada elemento se guardará como etiqueta.",
            label_visibility="collapsed",
        )
        temas_lista = normalizar_temas(temas_captura)
        mostrar_chips_temas(temas_lista)
        temas_tratados = temas_a_texto(temas_lista)
...
def formulario_seguimiento_interacciones(registro=None, id_interaccion_preseleccionada=""):
    ...
    with st.form("form_seguimiento_interacciones", clear_on_submit=False):
        ...
        with col1:
            ...
            id_seguimiento = st.text_input("ID seguimiento", value=registro.get("id_seguimiento", generar_id("SEG")), disabled=True, help="ID automático generado por el sistema.", label_visibility="collapsed")
...
def formulario_participantes(registro=None):
    ...
    with st.form("form_participantes", clear_on_submit=False):
        ...
        with col1:
            ...
            id_participante = st.text_input("ID participante", value=registro.get("id_participante", generar_id("PART")), disabled=True, help="ID automático generado por el sistema.", label_visibility="collapsed")
            ...
            etiqueta_campo("participante_en_base", incompletos)
            valor_en_base = registro.get("participante_en_base", "Sí" if registro.get("id_persona") or registro.get("id_actor") else "No")
            participante_en_base = st.selectbox(
                "¿El participante está en la base?",
                catalogos["booleano"],
                index=indice_opcion(catalogos["booleano"], valor_en_base),
                label_visibility="collapsed",
            )

            origen_base = "Persona"
            if participante_en_base == "Sí":
                origen_default = "Actor clave" if registro.get("id_actor") else "Persona"
                origen_base = st.selectbox(
                    "Origen del participante en base",
                    ["Persona", "Actor clave"],
                    index=indice_opcion(["Persona", "Actor clave"], origen_default),
                    help="Selecciona si el participante se toma del catálogo de personas o de actores clave.",
                )
        ...
        with col2:
            ...
            id_actor = ""
            nombre_participante_externo = ""
            rol_default = registro.get("rol_participante", "")

            if participante_en_base == "Sí" and origen_base == "Persona":
                etiqueta_campo("id_persona", incompletos)
                opciones_persona = obtener_opciones_columna(personas, "id_persona")
                id_persona = st.selectbox("ID persona", opciones_persona, index=indice_opcion(opciones_persona, registro.get("id_persona", "")), label_visibility="collapsed")
                persona = obtener_registro_por_id(personas, "id_persona", id_persona)
                nombre_participante_externo = persona.get("nombre_persona", "") if persona else ""
                rol_default = persona.get("relacion_hogar", rol_default) if persona else rol_default
                st.text_input("Nombre del participante", value=nombre_participante_externo, disabled=True)

            elif participante_en_base == "Sí" and origen_base == "Actor clave":
                etiqueta_campo("id_actor", incompletos)
                ...
                actor = obtener_registro_por_id(actores, "id_actor", id_actor)
                nombre_participante_externo = actor.get("nombre_actor", "") if actor else ""
                rol_default = actor.get("tipo_actor", rol_default) if actor else rol_default
                st.text_input("Nombre del participante", value=nombre_participante_externo, disabled=True)

            else:
                etiqueta_campo("nombre_participante_externo", incompletos)
                nombre_participante_externo = st.text_input(
                    "Nombre participante externo",
                    value=registro.get("nombre_participante_externo", ""),
                    help="Captura manualmente el nombre cuando el participante no existe en la base.",
                    label_visibility="collapsed",
                )
        ...
        rol_participante = st.text_input("Rol participante", value=rol_default, label_visibility="collapsed")
        ...
        if guardar:
            nuevo_registro = {
                ...
                "id_interaccion": id_interaccion,
                "participante_en_base": participante_en_base,
                "id_persona": id_persona,
                "id_actor": id_actor,
...
def pantalla_actores_clave(filtro_actor, filtro_zona):
    """Pantalla de gestión de actores clave."""
    st.subheader("Actores clave")
    df = aplicar_filtros(st.session_state.actores_clave, filtro_actor, filtro_zona)
    ...
    if indice is not None and not df.empty:
        ...
        mostrar_descarga_registro_seleccionado(registro, "m02_actor", "Actor clave seleccionado", "actor seleccionado")
...
def pantalla_interacciones(filtro_actor, filtro_zona):
    """Pantalla de gestión de interacciones."""
    st.subheader("Interacciones")
    df = aplicar_filtros(st.session_state.interacciones, filtro_actor, filtro_zona)
    columnas = ["id_interaccion", "fecha_interaccion", "tipo_interaccion", "canal", "motivo", "requiere_seguimiento", "resultado", "id_hogar", "id_lugar_poblado"]
    ...
    if indice is not None and not df.empty:
        ...
        mostrar_descarga_registro_seleccionado(registro, "m02_interaccion", "Interacción seleccionada", "registro de interacción seleccionado")
...
def pantalla_seguimiento_interacciones(filtro_actor, filtro_zona):
    ...
    interacciones_filtradas = aplicar_filtros(st.session_state.interacciones, filtro_actor, filtro_zona)
    ...
    if filtro_actor or filtro_zona:
        df = df[df["id_interaccion"].astype(str).isin(interacciones_filtradas["id_interaccion"].astype(str).tolist())]
    ...
    if indice is not None and not df.empty:
        ...
        mostrar_descarga_registro_seleccionado(registro, "m02_seguimiento", "Seguimiento seleccionado", "seguimiento seleccionado")
...
def pantalla_participantes(filtro_actor, filtro_zona):
    ...
    interacciones_filtradas = aplicar_filtros(st.session_state.interacciones, filtro_actor, filtro_zona)
    if filtro_actor or filtro_zona:
        ids_interacciones = interacciones_filtradas["id_interaccion"].astype(str).tolist()
        mascara_interacciones = df["id_interaccion"].astype(str).isin(ids_interacciones)
        mascara_actor = df.get("id_actor", pd.Series("", index=df.index)).astype(str).eq(str(filtro_actor)) if filtro_actor else False
        df = df[mascara_interacciones | mascara_actor]

    columnas = ["id_participante", "id_interaccion", "participante_en_base", "id_persona", "id_actor", "nombre_participante_externo", "tipo_participante", "rol_participante", "firma_asistencia"]
    ...
    if indice is not None and not df.empty:
        ...
        mostrar_descarga_registro_seleccionado(registro, "m02_participante", "Participante seleccionado", "participante seleccionado")
...
def main():
    ...
    filtro_actor, filtro_zona = mostrar_filtros_globales()
    st.divider()

    if tabla == "Actores clave":
        pantalla_actores_clave(filtro_actor, filtro_zona)
    elif tabla == "Interacciones":
        pantalla_interacciones(filtro_actor, filtro_zona)
    elif tabla == "Seguimiento de interacciones":
        pantalla_seguimiento_interacciones(filtro_actor, filtro_zona)
    else:
        pantalla_participantes(filtro_actor, filtro_zona)
