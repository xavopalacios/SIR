...
def crear_mapa_predio_pdf(registro_predio):
    ...
    mapa = crear_mapa_base(lat=centro[0], lon=centro[1], zoom=19)
...
def generar_imagen_predio_png(registro_predio):
    ...
    # Intento principal: captura real del mapa Folium con base satelital y etiquetas.
    try:
        mapa = crear_mapa_predio_pdf(registro_predio)
        png_bytes = mapa._to_png(delay=5)
    ...
    # Segundo intento: guardar HTML Folium y capturar con Selenium si está disponible.
    try:
    ...
    # Respaldo cartográfico sin ejes ni cuadrícula. Se usa solo si el entorno no permite capturar Folium.
...
def crear_mapa_base(lat=9.19, lon=-80.10, zoom=11):
    ...
    Nota técnica:
    - max_zoom permite que el usuario acerque más el mapa en la interfaz.
    - max_native_zoom indica hasta qué nivel la capa tiene teselas reales.
    - Cuando se supera el max_native_zoom, Leaflet sobreescala la imagen disponible.
    """
    mapa = folium.Map(
        location=[lat, lon],
        zoom_start=zoom,
...
def mostrar_ficha_registro(tabla, registro):
    ...
    # Activos afectados: renderizado independiente para evitar que Streamlit escape HTML largo como texto.
    if tabla == "predios":
        activos_predio = obtener_activos_por_predio(registro.get("id_predio", ""))
        if not activos_predio.empty:
            st.markdown("#### Activos afectados asociados")
            cols = st.columns(2)
            for idx, (_, activo) in enumerate(activos_predio.iterrows()):
                with cols[idx % 2]:
                    st.markdown(
                        f"""
...
def mostrar_mapa_predio_seleccionado(id_predio=None, df_contexto=None):
    ...
    if id_predio and not predios.empty:
        ...
        if not predios_sel.empty:
            ...
            if isinstance(vertices, str):
                try:
                    vertices_bounds = json.loads(vertices)
                except Exception:
                    vertices_bounds = []
            else:
                vertices_bounds = vertices
            if isinstance(vertices_bounds, list) and len(vertices_bounds) >= 3:
                mapa.fit_bounds(vertices_bounds)
