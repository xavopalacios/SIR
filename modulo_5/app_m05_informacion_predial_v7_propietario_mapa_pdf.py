...
ESQUEMA_M05 = {
    ...
    "predios": {
        ...
        "campos_principales": [
            ...
            "area_total_m2", "area_afectada_m2", "porcentaje_afectacion", "estado_liberacion"
        ],
        "campos": {
...
def generar_imagen_predio_png(registro_predio):
    ...
    ax.set_title(
        f"Mapa del predio {registro_predio.get('id_predio', '')}",
        fontsize=12,
        weight="bold",
        color="#073B5A",
    )
...
def agrupar_campos_ficha(tabla, registro):
    ...
    for campo in ESQUEMA_M05[tabla]["campos"]:
        if campo in ["vertices_poligono", "numero_vertices"]:
            continue
...
def mostrar_tabla_mapa_y_ficha_predios(filtros):
    ...
    st.download_button(
        ...
        data=convertir_para_visualizacion(df_filtrado.drop(columns=[c for c in ["vertices_poligono", "numero_vertices"] if c in df_filtrado.columns])).to_csv(index=False).encode("utf-8-sig"),
...
def mostrar_tabla_y_ficha(tabla, filtros):
    ...
    llave = config["llave"]
    df_filtrado = filtrar_dataframe(tabla, filtros)
    campos = [c for c in config["campos_principales"] if c in df_filtrado.columns and c not in ["vertices_poligono", "numero_vertices"]]
