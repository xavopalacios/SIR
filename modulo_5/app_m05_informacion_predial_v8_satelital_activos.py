...
def generar_pdf_ficha(tabla, registro):
    ...
    if tabla == "predios":
        activos_txt = resumir_activos_por_predio(registro.get("id_predio", ""))
        if activos_txt:
            data_tabla.append(["Activos afectados asociados", ""])
            for tipo_activo in [t.strip() for t in activos_txt.split(";") if t.strip()]:
                data_tabla.append(["Tipo de activo", tipo_activo])
...
def crear_data_inicial():
    ...
    for i, predio in enumerate(predios[:12], start=1):
        activos.append({
            ...
            "tipo_geometria": clasificar_geometria_activo(["Vivienda", "Cultivo", "Cerca", "Pozo", "Mejora", "Árbol"][i % 6]),
            "geometria_activo": "",
...
def crear_mapa_base(lat=9.19, lon=-80.10, zoom=11):
    """Crea mapa base satelital para todas las visualizaciones cartográficas del módulo."""
    mapa = folium.Map(location=[lat, lon], zoom_start=zoom, tiles=None, control_scale=True)

    # Base satelital pública de Esri World Imagery.
    folium.TileLayer(
...
def agregar_activos_al_mapa(mapa, activos, predio_row=None):
    ...
    for idx, (_, row) in enumerate(activos.iterrows()):
        ...
        popup = f"""
        <b>Activo:</b> {row.get('id_activo_afectado', '')}<br>
        ...
        if tipo_geom == "punto":
            folium.CircleMarker(
                ...
                tooltip=f"{row.get('id_activo_afectado', '')} · {tipo_activo}",
            ).add_to(mapa)
        ...
        if activos_txt:
            ...
            for tipo_activo in [t.strip() for t in activos_txt.split(";") if t.strip()]:
                ...
                <div class="record-field">
                    ...
                    <div class="record-value">{escape(tipo_activo)}</div>
                </div>
                """
            html += "</div>"
