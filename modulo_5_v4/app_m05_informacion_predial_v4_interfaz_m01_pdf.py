    ...
    for grupo, campos in agrupar_campos_ficha(tabla, registro).items():
        ...
        for campo in campos:
            if (tabla, campo) in RELACIONES:
                valor_txt = resolver_contexto_relacional(tabla, campo, registro.get(campo))
            else:
                valor_txt = formatear_valor(campo, registro.get(campo))
            data_tabla.append([etiqueta_campo(campo), str(valor_txt)])
