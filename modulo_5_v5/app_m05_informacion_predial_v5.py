...
ESQUEMA_M05 = {
    ...
    "predios": {
        ...
        "llave": "id_predio",
        "campos_principales": [
            "id_predio", "id_hogar", "id_lugar_poblado", "uso_principal", "tipo_tenencia",
            "area_total_m2", "area_afectada_m2", "porcentaje_afectacion",
            "numero_vertices", "estado_liberacion"
        ],
        "campos": {
    ...
    "infraestructura_comunitaria": {
        ...
        "campos_principales": [
            "id_infraestructura", "id_lugar_poblado", "nombre_infraestructura",
    ...
    "avaluos": {
        ...
        "llave": "id_avaluo",
        "campos_principales": [
            "id_avaluo", "id_predio", "id_hogar", "id_activo_afectado",
...
def cargar_hogares_desde_modulo_i():
    """Carga hogares y lugares poblados desde la memoria local del M01 cuando exista."""
...
def sincronizar_hogares_desde_modulo_i():
    ...
    hogares_m01, lugares_m01 = cargar_hogares_desde_modulo_i()
...
def generar_imagen_predio_png(registro_predio):
    """Genera una imagen PNG esquemática del predio consultado a partir de su polígono."""
    ...
    xs = [float(v[1]) for v in vertices]  # lon
    ys = [float(v[0]) for v in vertices]  # lat
    ...
    ax.set_title(f"Predio {registro_predio.get('id_predio', '')}")
    ...
    if centro:
        ax.text(float(centro[1]), float(centro[0]), str(registro_predio.get("id_predio", "")),
...
def generar_pdf_ficha(tabla, registro):
    ...
    if predio_relacionado:
        ...
        if img_buffer:
            elementos.append(Paragraph("Imagen / vista esquemática del predio consultado", styles["Heading3"]))
...
def crear_data_inicial():
    ...
    hogares_importados, lugares_importados = cargar_hogares_desde_modulo_i()
    ...
    infra = pd.DataFrame([
        {"id_infraestructura": "INF-0001", "id_lugar_poblado": "COM-0001", "nombre_infraestructura": "Escuela comunitaria", "tipo_infraestructura": "Educativa", "estado_fisico": "Regular", "uso_actual": "Activo", "responsable_comunitario": "Comité escolar", "requiere_reposicion": "Sí", "lat": 9.1908, "lon": -80.0874, "observaciones": "Requiere revisión de acceso y servicios básicos."},
        {"id_infraestructura": "INF-0002", "id_lugar_poblado": "COM-0002", "nombre_infraestructura": "Casa comunal", "tipo_infraestructura": "Comunitaria", "estado_fisico": "Bueno", "uso_actual": "Activo", "responsable_comunitario": "Junta local", "requiere_reposicion": "No", "lat": 9.2024, "lon": -80.0991, "observaciones": "Punto de reunión comunitaria."},
        {"id_infraestructura": "INF-0003", "id_lugar_poblado": "COM-0003", "nombre_infraestructura": "Pozo comunitario", "tipo_infraestructura": "Agua", "estado_fisico": "Malo", "uso_actual": "Limitado", "responsable_comunitario": "Comité de agua", "requiere_reposicion": "Sí", "lat": 9.1662, "lon": -80.1369, "observaciones": "Fuente de agua con mantenimiento pendiente."},
        {"id_infraestructura": "INF-0004", "id_lugar_poblado": "COM-0004", "nombre_infraestructura": "Capilla", "tipo_infraestructura": "Religiosa", "estado_fisico": "Regular", "uso_actual": "Activo", "responsable_comunitario": "Comunidad", "requiere_reposicion": "Por evaluar", "lat": 9.2065, "lon": -80.0616, "observaciones": "Uso social y ceremonial."},
        {"id_infraestructura": "INF-0005", "id_lugar_poblado": "COM-0005", "nombre_infraestructura": "Puente peatonal", "tipo_infraestructura": "Vial", "estado_fisico": "Malo", "uso_actual": "Limitado", "responsable_comunitario": "Comunidad", "requiere_reposicion": "Sí", "lat": 9.2144, "lon": -80.1137, "observaciones": "Infraestructura crítica para acceso local."},
    ])

    activos = []
    for i, predio in enumerate(predios[:12], start=1):
        activos.append({
            "id_activo_afectado": f"AAF-{i:04d}",
            "id_predio": predio["id_predio"],
            "id_hogar": predio["id_hogar"],
            "tipo_activo": ["Vivienda", "Cultivo", "Cerca", "Pozo", "Mejora", "Árbol"][i % 6],
            "descripcion_activo": f"Activo simulado asociado al predio {predio['id_predio']}.",
            "cantidad": float(1 + i),
            "unidad_medida": ["Unidad", "m2", "ha", "metro lineal", "global"][i % 5],
            "estado_conservacion": ["Bueno", "Regular", "Malo", "No evaluado"][i % 4],
            "evidencia_fotografica": f"DOC-FOTO-{i:04d}",
...
def validar_registro(tabla, registro):
    ...
    if tabla == "predios":
        ...
        if numero_vertices(registro.get("vertices_poligono")) < 3:
            errores.append("El polígono debe tener al menos 3 vértices.")
...
def pantalla_inicio(filtros):
    ...
    if not predios.empty:
        resumen = predios.groupby("id_hogar", dropna=False).agg(
            ...
            vertices_promedio=("numero_vertices", "mean"),
        ).reset_index()
    ...
    st.markdown("#### Ejemplos de polígonos irregulares")
    ejemplos = predios[["id_predio", "id_hogar", "area_total_m2", "area_afectada_m2", "porcentaje_afectacion", "numero_vertices"]].head(10) if not predios.empty else pd.DataFrame()
    st.dataframe(ejemplos, use_container_width=True, hide_index=True)
    st.caption("Los predios simulados incluyen polígonos de 5 a 12 vértices, con áreas y niveles de afectación distintos para pruebas de interacción.")
