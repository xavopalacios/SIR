...
# Versión v12 - Proceso documental, relaciones depuradas y datos de ejemplo
# ============================================================
# Base funcional:
...
def pantalla_documentos_proyecto() -> None:
    ...
    fechas_proceso = pd.to_datetime(
        sub.get("fecha_limite_proceso", pd.Series(dtype=str)),
        errors="coerce",
    )
    procesos_vencidos = int((fechas_proceso.dt.date < hoy).sum()) if not fechas_proceso.empty else 0
    procesos_proximos = int(
        (
            (fechas_proceso.dt.date >= hoy)
            & (fechas_proceso.dt.date <= hoy + timedelta(days=30))
        ).sum()
    ) if not fechas_proceso.empty else 0
