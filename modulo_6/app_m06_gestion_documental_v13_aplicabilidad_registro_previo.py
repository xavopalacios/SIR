...
# Versión v13.1 - Proceso documental, relaciones depuradas y datos de ejemplo
# ============================================================
# Base funcional:
...
def crear_datos_operativos_ejemplo() -> dict[str, pd.DataFrame]:
    ...
    for i in range(1, 11):
        ...
        if cumple_proceso:
            # Casos variados: vencido, próximo a vencer y con plazo amplio.
            if i in (2, 8):
                fecha_limite_proceso = (hoy - timedelta(days=5)).isoformat()
            elif i in (4, 10):
                fecha_limite_proceso = (hoy + timedelta(days=20)).isoformat()
            else:
                fecha_limite_proceso = (hoy + timedelta(days=90 + i)).isoformat()
        else:
        ...
        documentos.append({
            ...
            "token_transaccion": f"TOKEN-{i:05d}",
    ...
    for i in range(1, 11):
        pnr_demo.append({
            ...
            "id_lugar_poblado": f"LPO-{((i - 1) % 10) + 1:03d}",
            "id_predio": f"PRE-{((i - 1) % 10) + 1:04d}",
        ...
        hsc_demo.append({
            ...
            "estado_identificacion": "Pendiente" if i % 2 else "En verificación",
        ...
        relaciones_caso_demo.append({
            ...
            "tipo_caso": "Persona no residente" if i % 2 else "Hogar sin censo",
            "id_caso_origen": f"PNR-{i:05d}" if i % 2 else f"HSC-{i:05d}",
...
def vista_documentos_proyecto_unificada(expediente: dict[str, Any]) -> None:
    """
    Índice compacto de documentos vigentes e históricos del proyecto.
    """
    actualizar_estados_vigencia()
    docs = st.session_state.data_m06["documentos"].copy()
    sub = docs[
    ...
    evento = st.dataframe(
        ...
        column_config={
            "id_documento": st.column_config.TextColumn("ID", width="small"),
            "tipo_documental": st.column_config.TextColumn(
                "Tipo documental", width="large"
            ),
            "carpeta": st.column_config.TextColumn("Carpeta", width="medium"),
            "fase": st.column_config.TextColumn("Fase", width="medium"),
            "version": st.column_config.NumberColumn("V.", width="small"),
            "estado_revision": st.column_config.TextColumn(
                "Revisión", width="small"
            ),
            "estado_vigencia": st.column_config.TextColumn(
                "Vigencia", width="small"
            ),
            "fecha_documento": st.column_config.DateColumn(
                "Fecha", width="small"
            ),
            "ruta_archivo": st.column_config.LinkColumn(
                "Archivo", display_text="Abrir", width="small"
            ),
        },
        key="tabla_documentos_proyecto_unificada",
    )
    ...
    if seleccion:
        ...
        seleccionado = st.selectbox(
            ...
            opciones,
            format_func=lambda valor: etiquetas.get(valor, valor),
            key="detalle_documento_proyecto_fallback",
        )
        indice = vista.index[
            vista["id_documento"].astype(str).eq(str(seleccionado))
        ][0]

    doc = vista.iloc[int(indice)].to_dict()
    st.markdown("#### Resumen del documento seleccionado")
    with st.container(border=True):
        c1, c2, c3 = st.columns(3)
        c1.metric("ID documento", doc.get("id_documento", ""))
