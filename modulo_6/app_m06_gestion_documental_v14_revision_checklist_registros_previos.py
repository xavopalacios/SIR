...
# Versión v14.1 - Proceso documental, relaciones depuradas y datos de ejemplo
# ============================================================
# Base funcional:
...
def crear_datos_operativos_ejemplo() -> dict[str, pd.DataFrame]:
    """
    Genera diez registros coherentes por tabla.

    Un documento solo existe cuando cuenta con archivo y ruta asociados.
    Las revisiones apuntan exclusivamente a documentos realmente cargados.
    """
    ...
    for i in range(1, 11):
        ...
        candidatos = catalogo[catalogo["nivel"].astype(str).eq(nivel)]
        item = (
            candidatos.iloc[(i - 1) % len(candidatos)]
            if not candidatos.empty
            else catalogo.iloc[(i - 1) % len(catalogo)]
        )
        ...
        if cumple_proceso:
            if i in (2, 8):
                ...
                fecha_limite_proceso = (hoy + timedelta(days=90)).isoformat()
        else:
        ...
        expedientes.append({
            ...
            "porcentaje_completitud": float(i * 10),
        ...
        nombre_archivo = f"documento_ejemplo_{i:02d}.pdf"
        ruta_archivo = f"https://repositorio.ejemplo.test/{nombre_archivo}"
        ...
        documentos.append({
            ...
            "usuario_carga": USUARIOS[(i + 1) % len(USUARIOS)],
            "usuario_revisor_asignado": USUARIOS[(i + 2) % len(USUARIOS)],
            "estado_revision": revision,
            "confirmado": confirmado,
            "version": 1,
            "observaciones_carga": f"Documento real de prueba {i}",
            "fecha_creacion": ahora(),
            "fecha_actualizacion": ahora(),
            "usuario_actualizacion": USUARIO_BETA,
        })
        ...
        id_caso = f"PNR-{i:05d}" if i % 2 else f"HSC-{i:05d}"
        ...
        historial_demo.append({
            ...
            "estado_nuevo": "En verificación" if i % 2 else "Identificado",
        ...
        evidencias_demo.append({
            ...
            "id_caso": id_caso,
            "tipo_evidencia": [
                "Fotografía", "Visita", "Intento de contacto",
                "Plano", "Información de terceros"
            ][(i - 1) % 5],
            "fecha_evidencia": hoy.isoformat(),
            "descripcion": f"Evidencia demostrativa {i}",
            "ruta_archivo": (
                f"https://repositorio.ejemplo.test/evidencia_{i:02d}.pdf"
            ),
...
def vista_checklist(expediente: dict[str, Any]) -> None:
    ...
    id_doc = st.selectbox(
        "Documento asociado para consultar",
        opciones,
        format_func=lambda valor: etiquetas.get(valor, valor),
