...
def cargar_datos_base():
    ...
    hogares_data = [
        ("HOG-0001", "Hogar Pérez", "LP-001", "Nuevo Paraíso", "Panamá Oeste", "Capira", "Río Indio", "Zona 1", 9.201, -80.085),
        ("HOG-0002", "Hogar González", "LP-002", "Santa Rosa", "Colón", "Chagres", "La Encantada", "Zona 1", 9.243, -80.131),
        ("HOG-0003", "Hogar Martínez", "LP-003", "Boca de Uracillo", "Panamá Oeste", "Capira", "Ciricito", "Zona 2", 9.154, -80.051),
        ("HOG-0004", "Hogar Rodríguez", "LP-004", "La Arenosa", "Panamá Oeste", "Capira", "Río Indio", "Zona 2", 9.291, -80.168),
        ("HOG-0005", "Hogar López", "LP-005", "El Limón", "Panamá Oeste", "Capira", "Río Indio", "Zona 3", 9.226, -80.102),
        ("HOG-0006", "Hogar Herrera", "LP-006", "Quebrada Bonita", "Panamá Oeste", "Capira", "Ciricito", "Zona 3", 9.173, -80.094),
        ("HOG-0007", "Hogar Torres", "LP-007", "Los Pinos", "Colón", "Chagres", "La Encantada", "Zona 1", 9.206, -80.062),
        ("HOG-0008", "Hogar Castillo", "LP-008", "Río Claro", "Colón", "Chagres", "La Encantada", "Zona 2", 9.214, -80.114),
        ("HOG-0009", "Hogar Díaz", "LP-009", "El Progreso", "Panamá Oeste", "Capira", "Río Indio", "Zona 3", 9.196, -80.090),
        ("HOG-0010", "Hogar Mendoza", "LP-010", "Nueva Esperanza", "Panamá Oeste", "Capira", "Río Indio", "Zona 1", 9.188, -80.080),
        ("HOG-0011", "Hogar prueba editable 01", "LP-011", "El Nance", "Panamá Oeste", "Capira", "Río Indio", "Zona 2", 9.232, -80.147),
        ("HOG-0012", "Hogar prueba editable 02", "LP-012", "Altos del Río", "Panamá Oeste", "Capira", "Ciricito", "Zona 3", 9.145, -80.121),
        ("HOG-0013", "Hogar prueba editable 03", "LP-013", "Las Mercedes", "Colón", "Chagres", "La Encantada", "Zona 1", 9.268, -80.105),
        ("HOG-0014", "Hogar prueba editable 04", "LP-014", "Quebrada Honda", "Panamá Oeste", "Capira", "Río Indio", "Zona 2", 9.181, -80.154),
    ]
    ...
    avaluos = pd.DataFrame([
        {
            ...
            "x": float(row["lon"] + 0.015),
            "y": float(row["lat"] + 0.012),
...
def agregar_scatter_y_texto(capas, datos, x_col="x", y_col="y", radio=115):
    if datos.empty:
        return capas
    datos = datos.dropna(subset=[x_col, y_col]).copy()
    ...
    if datos.empty:
        return capas
    capas.append(pdk.Layer("ScatterplotLayer", data=datos, get_position=f"[{x_col}, {y_col}]", get_fill_color="color", get_radius=radio, pickable=True, opacity=0.88))
    capas.append(pdk.Layer("TextLayer", data=datos, get_position=f"[{x_col}, {y_col}]", get_text="titulo_mapa", get_size=13, get_color=[20, 35, 45], get_angle=0, get_text_anchor="middle", get_alignment_baseline="bottom", pickable=False))
    return capas
...
def mapa_dos_puntos(componente, bien):
    ...
    if componente:
        rows.append({
            ...
            "y": componente.get("y", componente.get("y_componente", "")), "color": [240, 90, 67, 190],
        })
    if bien:
        rows.append({
            ...
            "descripcion_visible": bien.get("descripcion_bien", ""), "x": bien.get("x"), "y": bien.get("y"), "color": [0, 166, 166, 210],
        })
...
def pantalla_bienes_reposicion():
    ...
    base = bien_existente.copy() if bien_existente else {
        ...
        "x": valor_float(componente.get("x"), -80.08) + 0.010,
        "y": valor_float(componente.get("y"), 9.20) + 0.010,
...
def pantalla_repositorio_bienes():
    """Repositorio visual de componente compensable vs bien de reposición."""
    st.markdown("### Repositorio de Bienes")
    st.markdown('<div class="screen-help">Consulta visual de la trazabilidad entre componente compensable y bien de reposición. Selecciona Hogar > Paquete > Bien de reposición.</div>', unsafe_allow_html=True)
    hogares = obtener_df("hogares")
    bienes = obtener_df("bienes_reposicion")
    if hogares.empty or bienes.empty:
        st.info("No hay bienes de reposición registrados para consultar.")
        return

    hogares_con_bienes = sorted(bienes["id_hogar"].dropna().astype(str).unique().tolist())
    id_hogar = st.selectbox("1. Hogar", hogares_con_bienes, key="repositorio_hogar")
    paquetes = bienes[bienes["id_hogar"].astype(str) == str(id_hogar)]["id_paquete_compensacion"].dropna().astype(str).unique().tolist()
    if not paquetes:
        st.info("El hogar seleccionado no tiene paquetes con bienes de reposición registrados.")
        return
    id_paquete = st.selectbox("2. Paquete", sorted(paquetes), key="repositorio_paquete")
    ...
    id_bien = st.selectbox("3. Bien de reposición", opciones_bien, format_func=lambda x: f"{x} · {bienes_filtrados[bienes_filtrados['id_bien_reposicion'].astype(str)==str(x)].iloc[0].get('numero_registro_componente', '')}")
    ...
    with col2:
        st.markdown("##### Bien de reposición")
    ...
    st.markdown("#### Mapa del componente seleccionado y bien de reposición")
...
def pantalla_infraestructura_comunitaria():
    ...
    st.markdown('<div class="screen-help">Pantalla para registrar o actualizar infraestructura comunitaria de reposición, jalando información base desde avalúos simulados.</div>', unsafe_allow_html=True)
    catalogos = cargar_catalogos()
    ...
    with st.expander("Filtros", expanded=True):
        col1, col2 = st.columns(2)
        filtro_lugar = col1.multiselect("Lugar poblado", sorted(infraestructura["nombre_lugar_poblado"].dropna().astype(str).unique().tolist())) if not infraestructura.empty else []
        filtro_estado = col2.multiselect("Estado del proceso", catalogos["estado_proceso"])
    filtrado = infraestructura.copy()
    if filtro_lugar:
        filtrado = filtrado[filtrado["nombre_lugar_poblado"].isin(filtro_lugar)]
    if filtro_estado:
        filtrado = filtrado[filtrado["estado_proceso"].isin(filtro_estado)]
...
def pantalla_inventario_entrega():
    ...
    st.markdown('<div class="screen-help">Registra inventario de entrega por bien de reposición. No se permite registrar dos veces el mismo bien; si ya existe, debe actualizarse.</div>', unsafe_allow_html=True)
    catalogos = cargar_catalogos()
    entregas = obtener_df("entregas_bienes")
    bienes = obtener_df("bienes_reposicion")

    mostrar_dataframe(entregas, ["id_entrega_bien", "fecha_registro", "id_bien_reposicion", "id_hogar", "id_paquete_compensacion", "estado_entrega", "fecha_entrega", "recibido_por", "conformidad_hogar", "acta_evidencia_entrega", "observaciones"], "Entregas registradas")
    if bienes.empty:
    ...
    id_hogar_default = base.get("id_hogar") if base.get("id_hogar") in hogares_bienes else (hogares_bienes[0] if hogares_bienes else "")
    idx_hogar = hogares_bienes.index(id_hogar_default) if id_hogar_default in hogares_bienes else 0

    with st.form(f"form_entrega_{modo}_{base.get('id_entrega_bien')}"):
        ...
        id_hogar = col2.selectbox("Filtro por hogar", hogares_bienes, index=idx_hogar)

        bienes_hogar = bienes[bienes["id_hogar"].astype(str) == str(id_hogar)]
        ids_bien = bienes_hogar["id_bien_reposicion"].astype(str).tolist()
        id_bien_default = base.get("id_bien_reposicion") if base.get("id_bien_reposicion") in ids_bien else (ids_bien[0] if ids_bien else "")
        idx_bien = ids_bien.index(id_bien_default) if id_bien_default in ids_bien else 0
        col3, col4 = st.columns(2)
        id_bien = col3.selectbox("ID de bien de reposición", ids_bien, index=idx_bien)
        estado = col4.selectbox("Estado de entrega", catalogos["estados_entrega"], index=catalogos["estados_entrega"].index(base.get("estado_entrega")) if base.get("estado_entrega") in catalogos["estados_entrega"] else 0)
        ...
        if estado == "Entregada":
            ...
            st.info("Los campos de entrega efectiva se habilitan únicamente cuando el estado sea Entregada.")
    ...
    if guardar:
        ...
        if estado == "Entregada" and not str(acta).strip().lower().startswith(("http://", "https://")):
